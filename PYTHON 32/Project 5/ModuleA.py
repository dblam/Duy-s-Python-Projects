# DUY B. LAM
# 61502602
# PROJECT 5 - GUI MODULE = IMPLEMENTS GAME INTERFACE
#
...
class inputDialog:
    ...
    def _on_ok_button(self) -> None:
        '''
            Function gathers user inputs for pregame specifications.
        '''
    ...
    def _on_cancel_button(self) -> None:
        '''
            Function allows user to cancel pregame specifications.
        '''
        self._dialog_window.destroy()

    def was_ok_clicked(self) -> bool:
        '''
            Function returns a bool value to indicate if okay button was cliked to gather user inputs.
        '''
        return self._ok_clicked

    def get_inputs(self) -> list:
        '''
            Function allows user to organize user inputs and return them in a list.
        '''
        temp_list = []
    ...
    def show(self) -> None:
        '''
            Function allows dialog box to be opened while user follow next steps.
        '''
...
class Layout:
    ...
    def start(self) -> None:
        '''
            Function starts GUI verision of Othello.
        '''
        self._root_window.mainloop()

    def _on_button1_clicked(self) -> None:
        '''
            Function is used to create new dialog box for pregame specifications.
            Also, it creates a new board game instance with user inputs.
        '''
        dialog = inputDialog()
        dialog.show()
        if dialog.was_ok_clicked() == True:
    ...
    def get_board_specifications(self) -> list:
        '''
            Function allows user to pass game specifications around using a get method that returns
...
class Board:
    ...
    def check_score_and_turn(self)-> None:
        '''
            Function checks current turn and score from Othello game logic and update
            to labels containing these information inside GUI of game instance.
        '''
    ...
    def _on_canvas_clicked(self, event: tkinter.Event) -> None:
        '''
            Function handles user clicks to specify their next move.
            If move is invalid, GUI will be unresponsive.
        '''
        #random = tkinter.messagebox.showinfo(message = 'clicked')
        canvas_width = self._canvas.winfo_width()
        canvas_height = self._canvas.winfo_height()
        self.width_frac = canvas_width / self._canvas_width
        self.height_frac = canvas_height / self._canvas_height

        row = math.floor(event.y / 40 / self.height_frac)
        column = math.floor(event.x / 40 / self.width_frac)

        userClicked = (row+1, column+1)
        board_filled = self.gameInstance.check_completeness()

        if board_filled == False:
            self.gameInstance.possible_valid_moves()#check if players have valid move
            if self.gameInstance.get_both_passes() == False:
                self.check_score_and_turn()
                self.gameInstance.next_move(userClicked)
                if self.gameInstance.get_directions_valid() == True:
                    print(self.gameInstance.get_directions_valid())
                    self.gameInstance.switch_turn()
        ...
        if self.gameInstance.get_both_passes() == True:
            winner = self.gameInstance.define_winner()
            self.check_score_and_turn()
            self.gameInstance.print_board()
            self._canvas.delete(tkinter.ALL)
            self.draw_rectangles()
            self.draw_circles()
            winner_message = tkinter.messagebox.showinfo(message = winner)
    ...
    def _on_canvas_resized(self, event: tkinter.Event) -> None:
        '''
            Function handles canvas resize to proportionately distribute change in window width and height.
            Canvas click targets and all it's widgets will also be adjusted accordingly.
        '''
        self._canvas.delete(tkinter.ALL)
    ...
    def draw_circle(self, x1, y1, x2, y2) -> None:
        '''
            Function takes a tuple of arguments to draw an oval inside canvas of game instance.
        '''
    ...
    def draw_circles(self):
        '''
            Function is used to draw all circles according to game board returned from game logic after every turn.
        '''
    ...
    def draw_rectangle(self, x1, y1, x2, y2) -> None:
        '''
            Function is used to draw a rectangle based on tuple of arguments.
        '''
    ...
    def draw_rectangles(self) -> None:
        '''
            Function uses user specifications to draw a blank board for the game instance.
        '''
    ...
    def show(self) -> None:
        '''
            Function allows new board game instance to be drawn and executed on another window until winner is declared.
        '''
